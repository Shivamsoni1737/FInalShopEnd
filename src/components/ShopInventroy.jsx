...
const Home = () => {
  ...
  return (
    <div className='m-12 mt-24'>
      <button
        className='border rounded flex row items-center py-2 px-4 gap-2'
        onClick={() => {
      ...
      <div className='flex gap-2'>
        {products &&
          products.map((item) => <ItemCards key={item.name} {...item} />)}
...
const ItemCards = (item) => {
  ...
  return (
    ...
    >
      {item.stock > 0 ? (
        <div className='border rela top-2 left-2 w-fit mt-4 rounded-full bg-green-500'>
          <p className='p-2 text-center'>In Stock</p>
        </div>
      ) : (
        <div className='border rounded-full bg-red-500'>
      ...
      <img
        src={item.image.url}
        alt={item.shopname}
        className='h-[200px] w-[200px] object-cover m-auto'
      />
      ...
      {item.stock ? (
        <div>
          <button className='w-full bg-green-500 p-2' onClick={handleAddtoCard}>
            Add to Cart
          </button>
        </div>
      ) : (
        <div className='text-red-500 p-2'>Out of Stock</div>
      )}
    </div>
  );
};
